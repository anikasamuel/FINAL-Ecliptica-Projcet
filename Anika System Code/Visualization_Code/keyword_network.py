...
# 100 universal agri/dev-related keywords
CLEAN_KEYWORDS = [
...
def generate_keyword_network(file_path, output_folder):
    ...
    # Use top 5 summary topics
    top_topics = df_exploded['Summary topic'].value_counts().head(5).index.tolist()
    df_filtered = df_exploded[df_exploded['Summary topic'].isin(top_topics)]

    # Map to clean keywords
    ...
    for _, row in df_filtered.iterrows():
        ...
        G.add_node(topic, type='topic', color=CATEGORY_COLORS['topic'])
        cat = get_category(keyword)
        G.add_node(keyword, type='keyword', color=CATEGORY_COLORS.get(cat, CATEGORY_COLORS['default']))
    ...
    for node, data in G.nodes(data=True):
        ...
        sizes.append(28 if data['type'] == 'topic' else 16)
